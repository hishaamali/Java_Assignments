public class Box {
	...
	public Box(int l, int w, int h) {// Constructor with parameters
		if ((l<1) || (w<1) || (h<1)) throw new IllegalArgumentException("Size must be greater than 1");
		length = l;
		width = w;
		height = h;
	}
	...
	public void setLength(int l) { // Sets length of Box, checks if it is valid. 
		if (l < 1) throw new IllegalArgumentException("Size must be greater than 1");
		l = length;
	}
	
	public void setWidth(int w) {	//Sets width of Box, checks if it is valid. 
		if (w < 1) throw new IllegalArgumentException("Size must be greater than 1");
		w = width;
	}
	
	public void setHeight(int h) { //Sets height of Box, checks if the size is valid.
		if (h < 1) throw new IllegalArgumentException("Size must be greater than 1");
		h = height;
	}
	...
	public int compareTo(Box other) {
		...
		if (this.Volume() < other.Volume()) return -1;
		if (this.Volume() == other.Volume()) return 0;
		return 0;
	}
